...
// workerResponse mirrors the Apps Script JSON envelope for httptest mocks.
...
func fakeCoalescer(t *testing.T, srv *httptest.Server) *appscript.Coalescer {
	t.Helper()
	return appscript.NewCoalescer(srv.Client(), []string{srv.URL}, srv.Listener.Addr().String(), "k", 5*time.Second)
}
...
func TestCoalescer_BatchesConcurrentRequests(t *testing.T) {
	...
	const n = 8
	coal := appscript.NewCoalescer(srv.Client(), []string{srv.URL}, srv.Listener.Addr().String(), "k", 5*time.Second)
...
func TestRelayRequestMulti_FailoverTransparent(t *testing.T) {
	...
	resp, err := appscript.RelayRequestMulti(
		client,
...
func TestFmtBytes(t *testing.T) {
	...
	for _, c := range cases {
		got := log.FmtBytes(c.in)
		if got != c.want {
			t.Errorf("log.FmtBytes(%d) = %q, want %q", c.in, got, c.want)
		}
	}
}
...
func TestLogf_RoutesToLogFunc(t *testing.T) {
	...
	log.SetLogFunc(func(level, msg string) {
		gotLevel = level
		gotMsg = msg
	})
	log.Logf("error", "test %d", 42)
	if gotLevel != "error" {
...
func TestWriteHTTPError(t *testing.T) {
	client, server := net.Pipe()
	go func() {
		conn.WriteHTTPError(server, http.StatusBadGateway, "some error")
		server.Close()
	}()
